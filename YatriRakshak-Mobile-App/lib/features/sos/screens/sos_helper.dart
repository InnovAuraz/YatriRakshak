...
class SosHelper {
  ...
  static Future<void> triggerSOSAlert() async {
    ...
    if (user == null) {
      print("❌ User not logged in. Trying emergency fallback...");
    ...
    try {
      ...
      if (snapshot.docs.isNotEmpty) {
        ...
        print("❌ No guardians found. Trying emergency fallback...");
        await _emergencyFallback();
      }
    } catch (e) {
  ...
  /// **Emergency Fallback - Call Emergency Services**
  static Future<void> _emergencyFallback() async {
    try {
      String locationLink = await getCurrentLocation();
      String message = "🚨 EMERGENCY! I need immediate help. My location: $locationLink";
      
      // Try to call emergency services (911, 112, or local emergency number)
      // You can customize this based on the region
      print("📞 Calling Emergency Services: 911");
      await _channel.invokeMethod("makeCall", {"phoneNumber": "911"});
      
      // Also try to send SMS to a default emergency contact if available
      // This could be a family member's number stored locally
